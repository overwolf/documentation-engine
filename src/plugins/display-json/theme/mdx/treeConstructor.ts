import { createTreeNode } from '../../../_base/mdx/baseBlockPlugin';
...
export const CreateDisplayJSON = (
  properties: Root,
  children: (dRecord | dComment)[],
): DisplayJSONNode => {
  return CreateDisplayJSONNode(
...
export const CreateCommentNode = (
  comment: DisplayJSONEscapeSequence,
): dComment => {
  switch (comment.sequence) {
    case ValidSequences['//']:
      return CreateDisplayJSONNode(DisplayJSONNodeTypes.CommentLine, {
        comment: comment.content,
      });
    case ValidSequences['/*']:
      return CreateDisplayJSONNode(DisplayJSONNodeTypes.CommentBlock, {
        comment: comment.content,
      });
    default:
      throw new Error(
        `Unsupported comment sequence! \`${comment.sequence}\` (\`${comment.content}\`)`,
      );
  }
};

export const CreateStringValueNode = (
  value: DisplayJSONEscapeSequence,
  comments: dComment[],
): dLiteral => {
  switch (value.sequence) {
    ...
    default:
      throw new Error(
        `Unsupported string sequence! \`${value.sequence}\` (\`${value.content}\`)`,
      );
  }
};

export const CreateExpandedTypeNode = (
  typeName: string,
  comments: dComment[],
  expandedType?: dArray | dObject,
): dType => {
  const children: [dObject | dArray | DisplayJSONTypeNode] = [
    CreateTypeNode(typeName),
  ];
  if (expandedType) children.push(expandedType);
  return TryCreateCommentedNode(
    CreateDisplayJSONNode(
      DisplayJSONNodeTypes.ExpandedType,
      undefined,
      children,
    ),
    comments,
...
export const CreateTopLevelRecordDefinition = (
...
export const CreateRecordNode = (
  fieldDefinition: dRecordDefinition,
  recordContent: dInnerValue,
  comments?: dComment[],
): dRecord => {
  return TryCreateCommentedNode(
...
export const CreateParametersNode = (
  ...
  return CreateDisplayJSONNode(
    DisplayJSONNodeTypes.Parameters,
    undefined,
    params,
  );
};
...
export const CreateObjectNode = (children: (dRecord | dComment)[]): dObject => {
  return CreateDisplayJSONNode(
    DisplayJSONNodeTypes.Object,
    undefined,
    children,
  );
};

export const TryCreateCommentedNode = <
  NodeType extends DisplayJSONNodeBase<any, any>,
>(
  node: NodeType,
  comments: dComment[],
): dCommented<NodeType> => {
...
export const CreateDisplayJSONNode = <
  Type extends DisplayJSONNodeTypes,
  Properties = undefined,
>(
  type: Type,
  properties: Properties,
  children?: DisplayJSONNodeBase<any, any>[],
): DisplayJSONNodeBase<Type, Properties> => {
