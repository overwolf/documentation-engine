import pluginFactory, {
  ...
  // ContentTransform,
  CreateContentNode,
} from '../../../_base/mdx/baseBlockPlugin';
import createContentIterator from '../../../_base/mdx/contentIterator';
...
import { EscapeType } from '../../../_base/mdx/escapeType';
...
import {
  ...
  dType,
  dRecord,
} from '../types/DisplayJSONNodes';
import DisplayJSONEscapeSequence, {
  ValidSequences,
} from '../enums/DisplayJSONEscapeSequence';
import { RecordTypes } from '../enums/DisplayJSONRecordTypes';
import {
  CreateArrayNode,
  CreateCommentNode,
  CreateDisplayJSON,
  CreateExpandedTypeNode,
  CreateFunctionNode,
  CreateObjectNode,
  CreateRecordDefinition,
  CreateRecordNode,
  CreateStringValueNode,
  CreateTopLevelRecordDefinition,
} from './treeConstructor';
...
const openingTagFactory: OpeningTagFactory<DisplayJSONOptions> = (
  options: DisplayJSONOptions,
) =>
  new RegExp(
    `^(?<!${options.escapeString})${openingTagFuzzy(
      options.openingTag,
    )}(?: ([\\w\\.\\-]*))?(?:\\s*)`,
  );
...
const closingTagFactory: ClosingTagFactory<DisplayJSONOptions> = (
  options: DisplayJSONOptions,
) =>
  new RegExp(
    `^(?<!${options.escapeString})${closingTagFuzzy(
      options.closingTag,
    )}(?:\\s*?)$`,
  );

const deEscapeOpening: DeEscapeOpening<DisplayJSONOptions> = (
  options: DisplayJSONOptions,
  escaped: string,
) =>
  escaped.replace(
    `${options.escapeString}(${openingTagFuzzy(options.openingTag)})`,
    '$1',
  );

const deEscapeClosing: DeEscapeClosing<DisplayJSONOptions> = (
  options: DisplayJSONOptions,
  escaped: string,
) =>
  escaped.replace(
    `${options.escapeString}(${closingTagFuzzy(options.openingTag)})`,
    '$1',
  );
...
// @ts-expect-error weird types error
const createContentNode: CreateContentNode<DisplayJSONOptions> = (
  options: DisplayJSONOptions,
  content: string[],
  matchGroups: string[],
): DisplayJSONNode => {
  const [, /* full */ name] = matchGroups as [string, string, string];
  const registry = new EscapeRegistry<SyntacticType, ValidSequences>(
    options._internalEscapePrefix,
  );
  const safeContent = escapeContent(registry, createContentIterator(content))
    .flatMap((value) => {
      return SyntaxSplit(value, /[\[\]\(\)\{\}]/);
    })
    .flatMap((value) => {
      return SyntaxSplit(value, /:|=>|=/);
    })
    .flatMap((value) => {
      return SyntaxSplit(value, /\s+/);
    })
    .flatMap((value) => {
      return SyntaxSplit(value, registry.NumberedSequenceMatcher());
    })
    .flatMap((value) => {
      return SyntaxSplit(value, ',').filter((isComma) => isComma !== ',');
    })
    .flatMap((value) => {
      return SyntaxSplit(value, ';').filter(
...
const ValidFieldName = (name: DisplayJSONEscapeSequence): boolean => {
  return (
    name.syntacticType === SyntacticType.StringValue ||
    (name.syntacticType === SyntacticType.Literal &&
      ValidLiteralFieldName(name))
  );
};
...
const ParseFull = (
  name: string,
  registry: DisplayJSONEscapeRegistry,
  ...
  while (!done) {
    try {
      if (record.syntacticType !== SyntacticType.Literal)
        throw new Error(
          `Invalid record in top level! ${JSON.stringify(
            record,
            undefined,
            4,
          )}`,
      ...
      if (recordType != record.content)
        console.warn(
          `Found capitalized display JSON block keyword ${record.content}`,
        );
      const recordName = TryGenerate(contentIterator);
      if (!ValidLiteralFieldName(LiteralToRecord(recordName)))
        throw new Error(
          `Invalid name for \`${record.content}\` \`${recordName}\`, name must contain only alphanumeric characters, and/or \`\\-\` and \`\\.\`!`,
        );
      let recordDefinition: dRecordDefinition, recordContent: dType | dFunction;
      let firstContentRecord: DisplayJSONEscapeSequence,
        leadingComments: dComment[];
      const enumRecordType: RecordTypes = RecordTypes[recordType];
      switch (enumRecordType) {
        case RecordTypes.function:
          // function {name}(...) => Type{...}
          // Read function
          recordDefinition = ParseRecordDefinition(
            LiteralToRecord(recordName),
            registry,
            contentIterator,
            { recordType: enumRecordType, unAssignable: true },
          );
          [firstContentRecord, leadingComments] = InnerParseUntilContent(
            registry,
            contentIterator,
            '',
          );
          if (
            LiteralToSyntacticType(firstContentRecord) !==
            SyntacticType.Function
          )
            throw new Error(
              `Mismatched record decleration! Can't start a ${recordType} record with a ${LiteralToSyntacticType(
                firstContentRecord,
              )} Decleration (Evaluated as ${firstContentRecord.content}).`,
            );
          recordContent = ParseFunction(
            registry,
            contentIterator,
            leadingComments,
          );
        ...
        case RecordTypes.game_info:
          ...
          if (syntacticType !== SyntacticType.Literal)
            throw new Error(
              `Mismatched record decleration! ${recordType} is not a type! (Evaluated as ${JSON.stringify(
                firstContentRecord,
                undefined,
                4,
              )}).`,
            );
          recordContent = ParseType(
            firstContentRecord,
            registry,
            contentIterator,
            leadingComments,
        ...
        default:
          throw new Error(
            `Invalid display JSON record keyword ${record.content}`,
          );
      }

      const recordNode = CreateRecordNode(
        recordDefinition,
        recordContent,
        comments,
      ); // group{definition, node}
...
const RecordToText = (
  record: DisplayJSONEscapeSequence,
  registry: DisplayJSONEscapeRegistry,
) => {
...
const ParseRecordDefinition = (
  name: DisplayJSONEscapeSequence,
  registry: DisplayJSONEscapeRegistry,
  contentIterator: Generator<string>,
  options?: ParseFieldFlags,
): dRecordDefinition => {
  const { recordType, unAssignable } = options ?? {};
  const assign = recordType ? '=' : ':';
  if (!(recordType ? ValidLiteralFieldName(name) : ValidFieldName(name)))
    throw new Error(
      `Invalid field name! ${name}, name must contain only alphanumeric characters ${
        recordType ? '' : ', and/or `-` and `.`'
      }!`,
    );
  if (!unAssignable) {
    const recordAssigner = TryGenerate(contentIterator);
    if (recordAssigner != assign)
      throw new Error(
        `Invalid assignment operator ${recordAssigner} for record name ${name}, expected ${assign}!`,
      );
  }

  return recordType
    ? CreateTopLevelRecordDefinition(recordType, RecordToText(name, registry))
    : CreateRecordDefinition(RecordToText(name, registry));
};

const InnerParseUntilContent = (
  registry: DisplayJSONEscapeRegistry,
  contentIterator: Generator<string>,
  error?: string,
): [DisplayJSONEscapeSequence, dComment[]] => {
...
const ParseUntilContent = (
  registry: DisplayJSONEscapeRegistry,
  contentIterator: Generator<string>,
): [boolean, DisplayJSONEscapeSequence, dComment[]] => {
  const comments = [];
...
const LiteralToSyntacticType = (
  record: DisplayJSONEscapeSequence,
): SyntacticType => {
  if (record.syntacticType !== SyntacticType.Literal)
    return record.syntacticType;
...
const ParseInnerValue = (
  firstValue: DisplayJSONEscapeSequence,
  leadingComments: dComment[],
  registry: DisplayJSONEscapeRegistry,
  contentIterator: Generator<string>,
): dInnerValue => {
  ...
  switch (syntacticType) {
    ...
    default:
      throw new Error(
        `Invalid value assignment! Cannot infer a value from ${JSON.stringify(
          firstValue,
          undefined,
          4,
        )}`,
...
const ParseFunction = (
  ...
  do {
    [value, leadingComments] = InnerParseUntilContent(
      registry,
      contentIterator,
    );
      ...
      default:
        parameters.push(
          CreateRecordNode(
            ...
            ParseInnerValue(
              ...InnerParseUntilContent(registry, contentIterator),
              registry,
              contentIterator,
            ),
            leadingComments,
  ...
  if (arrow !== '=>')
    throw new Error(`Invalid lambda token! Expected =>, but got ${arrow}`);

  const [typeRecord, typeComents] = InnerParseUntilContent(
    registry,
    contentIterator,
  );
...
const ParseArray = (
  ...
  do {
    ...
    switch (value.content) {
      ...
      default:
        children.push(
          ParseInnerValue(value, leadingComments, registry, contentIterator),
        );
        break;
    }
  } while (value);
...
const ParseType = (
  ...
  if (!typeMatcher.exec(type))
    throw new Error(
      `${JSON.stringify(record, undefined, 4)} is not a valid type syntax!`,
    );

  let expandedType: dArray | dObject;
  if (!primitiveTypes.includes(type)) {
    const expandedStart = TryGenerate(
      contentIterator,
      `${type} is not defined as an accepted primitive type! ${
        primitiveTypes.filter(
          (primitive) =>
            primitive.toLocaleLowerCase() === type.toLocaleLowerCase(),
        )
          ? 'Mind the capitalization!'
          : 'Maybe there is a typo somewhere?'
      }`,
    );
    const expandedSyntacticType = LiteralToSyntacticType(
      LiteralToRecord(expandedStart),
    );
    switch (expandedSyntacticType) {
      ...
      default:
        throw new Error(
          `Invalid suffix for type name ${expandedStart}, resulted in SyntacticType of ${expandedSyntacticType}`,
        );
    }
  }
...
const findEscapeSequence = (currentLine: string): EscapeSequence => {
  ...
  escapeSequences.forEach((value) => {
    ...
    [];
    if (newIndex < escapeStart.index)
      escapeStart = { index: newIndex, consumer: value.handleEscape };
  });
  return escapeStart;
};
...
const escapeContent = (
  ...
  try {
    while (
      !(lastFullLine = contentIterator.next() as IteratorYieldResult<string>)
        .done
    ) {
      ...
      do {
        ...
        if (escapeStart.consumer) {
          currentLine = escapeStart.consumer(
            currentLine,
            registry,
            escapeStart.index,
            contentIterator,
    ...
    throw new Error(`Failed to parse string, escaped sequence could not be terminated. Started at ${
      lastFullLine.value
    }!
