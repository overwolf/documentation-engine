...
export const DefaultTaggerOptions: TaggerOptions = {
  tag: '@@',
  keywords: ['Function', 'Object', 'Enum', 'Event', 'GameEvent', 'InfoUpdate'],
};

function normalizeOptions(options: Partial<TaggerOptions>): TaggerOptions {
...
const codeComponentTagger: Plugin = function plugin(
  ...
  function blockTokenizer(this: any, eat: any, value: string, silent: boolean) {
    ...
    const now = eat.now();
    const [opening, keyword] = match as string[] as [string, string];
    const food: string[] = [];
